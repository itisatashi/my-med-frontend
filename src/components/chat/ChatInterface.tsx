...
import { getDoctorSpecialization, getSpecializationRussianName } from '../../data/doctors';
...
const ChatInterface: React.FC = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await Promise.race([
        fetch('/api/api.php', { // This will be proxied to https://begdulla.uz/APII/api.php
          method: 'POST',
          headers: {
      ...
      if (response.ok) {
        ...
        if (responseText.length > 20) { // Only analyze substantial responses
          // Extract likely symptoms from user query
          const potentialSymptoms = userQuery.split(/[,.;!?\s]+/).filter(word => word.length > 3);
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setInputValue(prev => (prev ? `${prev} [Файл: ${fileNames}]` : `[Файл: ${fileNames}]`));
  };
    ...
    <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <Paper 
        ...
        sx={{ 
          ...
          backgroundColor: theme.palette.mode === 'dark' ? 'background.paper' : '#f5f9fc',
          mb: 2
        }}
      >
        <Avatar 
      ...
      >
        {messages.map((message) => (
          ...
          >
            <Box sx={{ display: 'flex', alignItems: 'flex-end', maxWidth: isMobile ? '90%' : '70%' }}>
              {message.sender === 'ai' && (
                <Avatar 
                  sx={{ 
                    bgcolor: 'primary.main', 
              ...
              >
                {message.loading ? (
                  ...
                  <Box sx={{ p: 2 }}>
                    ...
                    {message.sender === 'ai' && message.doctorRecommendation && (
                      <Box sx={{ mt: 2 }}>
                        <Alert 
                          severity="info" 
                        ...
                        >
                          ...
                          <Typography variant="body2" paragraph>
                            По вашим симптомам, рекомендуем консультацию у специалиста: <b>{getSpecializationRussianName(message.doctorRecommendation.specialization || '')}</b>
                          </Typography>
                          <Button
                            variant="outlined"
                            size="small"
                            endIcon={<ArrowForwardIcon />}
                            onClick={() => navigate(`/consult?specialization=${message.doctorRecommendation?.specialization || ''}`)}
                            sx={{ borderRadius: 4 }}
                          >
                            Найти врача
                          </Button>
            ...
            {message.sender === 'ai' && message.suggestions && message.suggestions.length > 0 && (
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mt: 1, maxWidth: '90%' }}>
      ...
      <Box
        ...
        sx={{ 
          p: 2, 
          bgcolor: theme.palette.mode === 'dark' ? 'background.paper' : '#f5f9fc',
          borderTop: '1px solid',
          borderColor: 'divider'
        }}
      >
